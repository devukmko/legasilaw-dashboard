...
export default function HomePage() {
  ...
  const {
    ...
    queryKey: ["feedbackData", page],
    queryFn: async (context) => {
      try {
        const feedbackResponse = await getAllFeedbacks(page, {
          supabase,
    ...
    <>
      ...
      <div className="p-6 bg-gray-100 min-h-screen">
        <Container>
          <Statistics
            counterData={{
              todayVisitors: counterData?.todayVisitors || 0,
              totalVisitors: counterData?.totalVisitors || 0,
              totalWhatsAppClicks: counterData?.totalWhatsAppClicks || 0,
            }}
          />
          <FeedbackSection
 ...
 */
async function fetchCounterData(context: {
  supabase: SupabaseClient;
  signal?: AbortSignal;
}) {
  try {
    return await getCounter(context);
  } catch (error) {
